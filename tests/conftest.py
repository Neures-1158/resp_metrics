...
def spontaneous_signal_df():
    ...
    for i, ti in enumerate(t):
        ...
        if t_in_cycle < Ti:
            ...
            if t_in_cycle < Ti/2:
                flow[i] = -0.5 * (t_in_cycle / (Ti/2))  # ramp down to -0.5
            else:
                flow[i] = -0.5 * (1 - (t_in_cycle - Ti/2) / (Ti/2))  # ramp up to 0
        else:
            # Expiration (positive flow) - triangular
            Te = cycle_period - Ti
            t_exp = t_in_cycle - Ti
            if t_exp < Te/2:
                flow[i] = 0.3 * (t_exp / (Te/2))  # ramp up to 0.3
            else:
                flow[i] = 0.3 * (1 - (t_exp - Te/2) / (Te/2))  # ramp down to 0
...
def mechanical_signal_df():
    ...
    - Ti = 0.8s, Te = 1.7s
    - Inspiratory flow: constant 0.6 L/s (square wave)
    - VT = 0.8 * 0.6 = 0.48 L
    - PEEP = 5 cmH2O
    - Ppeak = 25 cmH2O
    ...
    - I:E ≈ 0.47
    - VT = 0.48 L
    - VE = 24 * 0.48 = 11.52 L/min
    - PIF = 0.6 L/s
    - PEEP = 5 cmH2O
    - Ppeak = 25 cmH2O
    - dP = 20 - 5 = 15 cmH2O (using Pplat)
    - Cstat = 0.48 / 15 = 0.032 L/cmH2O
    """
    fs = 100  # Hz
    duration = 10.0
    t = np.arange(0, duration, 1/fs)
    ...
    Ti = 0.8
    Ti_flow = 0.6  # Active flow phase
    Ti_plateau = 0.2  # Plateau phase (no flow)
    ...
    for i, ti in enumerate(t):
        ...
        if t_in_cycle < Ti_flow:
            ...
            flow[i] = -0.4 * np.exp(-3 * t_exp / Te)
            # Pressure returns to PEEP
            pressure[i] = 5 + 15 * np.exp(-5 * t_exp / Te)
