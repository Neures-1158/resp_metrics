...
class TestComputeFromLabchartSpontaneous:
    ...
    @patch("resp_metrics.api.LabChartFile", MockLabChartFile)
    def test_block_column_first_spontaneous(self):
        """Block should be the first column in output DataFrames."""
        result = compute_from_labchart(
            "test.txt",
            flow_col="Flow",
            flow_unit="L/min",
        ...
        assert result["cycles"].columns[0] == "block_name"
        assert result["ventilatory"].columns[0] == "block_name"
...
class TestComputeFromLabchartMechanical:
    ...
    @patch("resp_metrics.api.LabChartFile", MockLabChartFile)
    def test_block_column_first_mechanical(self):
        """Block should be the first column in output DataFrames."""
        result = compute_from_labchart(
            "test.txt",
            flow_col="Flow",
        ...
        assert result["cycles"].columns[0] == "block_name"
        assert result["ventilatory"].columns[0] == "block_name"
        assert result["ventilator"].columns[0] == "block_name"


class TestComputeFromLabchartSaving:
    """Tests for CSV output saving."""

    @patch("resp_metrics.api.LabChartFile", MockLabChartFile)
    def test_saves_outputs_with_default_prefix(self, tmp_path):
        """Should save CSV files using the input file stem as prefix."""
        ...
        assert (tmp_path / "test_cycles_block1.csv").exists()
        assert (tmp_path / "test_metrics_block1.csv").exists()
        assert (tmp_path / "test_ventilator_block1.csv").exists()
    ...
    def test_mechanical_without_pressure_no_ventilator(self):
        ...
        with pytest.warns(RuntimeWarning, match="pressure_col is None"):
            result = compute_from_labchart(
                "test.txt",
                flow_col="Flow",
                flow_unit="L/min",
    ...
    @patch("resp_metrics.api.LabChartFile", MockLabChartFile)
    def test_mechanical_unavailable_warns(self, monkeypatch):
        """Mechanical mode should warn if mechanical metrics are unavailable."""
        import resp_metrics.api as api

        monkeypatch.setattr(api, "_HAS_VENTILATOR", False)
        monkeypatch.setattr(
            api, "_VENTILATOR_IMPORT_ERROR", RuntimeError("mock import error")
        )
        monkeypatch.setattr(api, "mechanical_from_cycles", None)

        with pytest.warns(
            RuntimeWarning, match="mechanical ventilation metrics are unavailable"
        ):
            result = compute_from_labchart(
                "test.txt",
                flow_col="Flow",
...
class TestComputeFromLabchartParameters:
    ...
    def test_block_selection(self):
        ...
        result = compute_from_labchart(
            "test.txt", block=1, flow_col="Flow", flow_unit="L/min"
        )
...
class TestComputeFromLabchartMultiBlock:
    ...
    def test_single_block_unchanged(self):
        ...
        assert result["ventilator"] is None or isinstance(
            result["ventilator"], pd.DataFrame
        )
    ...
    def test_multiple_blocks_list(self):
        ...
        result = compute_from_labchart(
            "test.txt", block=[1, 2], flow_col="Flow", flow_unit="L/min"
        )
    ...
    def test_all_blocks_none(self):
        ...
        result = compute_from_labchart(
            "test.txt", block=None, flow_col="Flow", flow_unit="L/min"
        )
